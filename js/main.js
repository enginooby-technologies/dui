...
if (!isStyleSheetAlreadyIncluded('animate.css'))
    loadStyleSheet(animateCss);
// if (document.querySelector('.fa') || document.querySelector('.fas'))
if (!isStyleSheetAlreadyIncluded('fontawesome.css'))
    loadStyleSheet(fontawesomeCss);
if (!isStyleSheetAlreadyIncluded('bootstrap.min.css'))
    loadStyleSheet(bootstrapMinCss, bootstrapMinCssIntegrity);
...
function isStyleSheetAlreadyIncluded(sheetName) {
    ...
    for (var i = 0; i < links.length; i++)
        if ((_a = links[i].getAttribute('href')) === null || _a === void 0 ? void 0 : _a.includes(sheetName))
            return true;
    return false;
}
